...
pub struct LeveledCompactionOptions {
    ...
    // base level is the level L0 compacts to. base level size must be <= base_level_size_mb
    // it means the cost of L0 compation to this level is acceptable
...
impl LeveledCompactionController {
    ...
    // used when compacting ssts of _sst_ids into level _in_level
    // return the sstids of _in_level that overlap with _sst_ids
    fn find_overlapping_ssts(
        ...
        let end_key = _sst_ids
            ...
            .cloned()
            .unwrap();
        // Note: in level compaction, L1+ is a sorted run
    ...
    pub fn generate_compaction_task(
        ...
        // target levels are levels that relate to compation, whose first postive size is the base level(<= base_level_size_bytes) for L0 compaction
        // and levels below that are over target size (L1+ compaction)
        let mut target_level_size = vec![0; self.options.max_levels]; // not including L0
        let mut real_level_size = Vec::with_capacity(self.options.max_levels);
        let mut base_level = self.options.max_levels;
        // compute real level size (not SST num)
        ...
        if _snapshot.l0_sstables.len() >= self.options.level0_file_num_compaction_trigger {
            println!("L0 compaction: compact L0 SST to base level {}", base_level);
        ...
        // calculate priority for L1 ~ Lmax-1
        for level in 0..self.options.max_levels - 1 {
            let prio: f64 = real_level_size[level] as f64 / target_level_size[level] as f64;
            if prio > 1.0 {
                // only compact when level is over target size
        ...
        if let Some((_, level)) = priority {
            println!(
                "L1+ compaction: target level sizes: {:?}, real level sizes: {:?}, base_level: {}",
                target_level_size
                    .iter()
    ...
    pub fn apply_compaction_result(
        ...
        // update upper level ssts (Note: new sst may be flushed to this level during compaction)
        ...
        // remove original sstables involved in compaction
        files_to_remove.extend(&_task.upper_level_sst_ids);
        files_to_remove.extend(&_task.lower_level_sst_ids);

        // update lower level ssts (keep unaffected ssts and add new compacted ssts)
        ...
        // sort lower level to maintain a sorted run (TODO: can we directly insert the range?)
